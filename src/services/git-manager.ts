  ...
  try {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Use execAsync instead of zx's $
        const { stdout, stderr } = await execAsync(command, { cwd: sourceDir });
...
export async function createGitCheckpoint(
  ...
  try {
    ...
    await executeGitCommandWithRetry(
      ['git', 'commit', '-m', `"${description} (attempt ${attempt})"`, '--allow-empty'],
      sourceDir,
      'creating commit'
    );
...
export async function getGitCommitHash(sourceDir: string): Promise<string | null> {
  ...
  try {
    const { stdout } = await execAsync('git rev-parse HEAD', { cwd: sourceDir });
    return stdout.trim();
  } catch {
    return null;
  }
